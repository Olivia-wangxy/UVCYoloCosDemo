...
//		mHandlerL = UVCCameraHandler.createHandler(this, mUVCCameraViewL, UVCCamera.DEFAULT_PREVIEW_WIDTH, UVCCamera.DEFAULT_PREVIEW_HEIGHT, BANDWIDTH_FACTORS[0]);
		mHandlerL = UVCCameraHandler.createHandler(this, mUVCCameraViewL, 1920, 1080, BANDWIDTH_FACTORS[1]);
	...
	private final OnClickListener mOnClickListener = new OnClickListener() {
		...
		public void onClick(final View view) {
			...
			if (id == R.id.camera_view_L) {
			...
			} else if (id == R.id.capture_button_L) {
				Log.d(TAG, "左边录制 onClick: ");
				if (mHandlerL != null) {
					if (mHandlerL.isOpened()) {
			...
			} else if (id == R.id.capture_button_R) {
				if (mHandlerR != null) {
					if (mHandlerR.isOpened()) {
						if (checkPermissionWriteExternalStorage() && checkPermissionAudio()) {
				...
				Log.d(TAG, "抓图 onClick: ");
				synchronized (mSync) {
					if (checkPermissionWriteExternalStorage()) {
						if ((mHandlerL != null) && mHandlerL.isOpened()) {
							final File outputFile = MediaMuxerWrapper.getCaptureFile(Environment.DIRECTORY_PICTURES, "_camera1.png");
							mHandlerL.captureStill(outputFile.toString());
						}
						if ((mHandlerR != null) && mHandlerR.isOpened()) {
							final File outputFile = MediaMuxerWrapper.getCaptureFile(Environment.DIRECTORY_PICTURES, "_camera2.png");
							mHandlerR.captureStill(outputFile.toString());
						}
					}
				}
	...
	private void saveBitmapToDocuments(Bitmap bitmap, String fileName) {
		String state = Environment.getExternalStorageState();
		if (!Environment.MEDIA_MOUNTED.equals(state)) {
			Toast.makeText(this, "SDCard 未挂载", Toast.LENGTH_SHORT).show();
			return;
		}

		File docsDir = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOCUMENTS), "MyApp");
		if (!docsDir.exists()) {
			docsDir.mkdirs();
		}
